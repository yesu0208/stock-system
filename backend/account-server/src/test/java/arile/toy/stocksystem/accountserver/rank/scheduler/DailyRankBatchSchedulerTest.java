...
class DailyRankBatchSchedulerTest {
    ...
    private void givenLock(Boolean acquired) {
        given(valueOps.setIfAbsent(LOCK_KEY, "LOCKED", Duration.ofMinutes(30))).willReturn(acquired);
    }
    ...
    void whenHoliday_skips() {
        ...
        verifyNoInteractions(valueOps, dailyRankBatchService, rankUpdatedPublisher);
    ...
    void whenLockNotAcquired_skips() {
        ...
        verifyNoInteractions(dailyRankBatchService, rankUpdatedPublisher);
        verify(redisTemplate, never()).delete(anyString());
    }
    ...
    void whenLockResultNull_skips() {
        ...
        verifyNoInteractions(dailyRankBatchService);
    }

    @Test
    @DisplayName("락을 잡으면 배치 실행 → 랭크 갱신 알림 발행 → 락 해제 순으로 처리한다")
    void success_runsPublishesAndReleasesLock() {
        ...
        InOrder inOrder = inOrder(dailyRankBatchService, rankUpdatedPublisher, redisTemplate);
        inOrder.verify(dailyRankBatchService).runDailyBatch();
        inOrder.verify(rankUpdatedPublisher).publish();
    ...
    void whenBatchFails_releasesLockWithoutPublishing() {
        given(holidayRegistry.isHoliday(any(LocalDate.class))).willReturn(false);
        givenLock(true);
        willThrow(new IllegalStateException("batch failed")).given(dailyRankBatchService).runDailyBatch();
