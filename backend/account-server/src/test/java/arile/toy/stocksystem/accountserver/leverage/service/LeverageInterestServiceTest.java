...
class LeverageInterestServiceTest {
    ...
    @Mock private LeverageInterestChargeExecutor leverageInterestChargeExecutor;

    private LeverageInterestService service;

    @BeforeEach
    void setUp() {
        service = new LeverageInterestService(
                leveragePositionRepository, new LeverageInterestCalculator(), leverageInterestChargeExecutor);
    }
    ...
    @Test
    @DisplayName("마지막 청구일부터 오늘까지의 경과 달력일수만큼 이자를 몰아서 청구하고 청구일을 오늘로 갱신한다")
    void chargesElapsedDays() {
        ...
        verify(leverageInterestChargeExecutor).chargeInterestForOnePosition(USERNAME, "005930", 1L, 255L);
    ...
    void whenNoLoan_skips(long loanAmount) {
        ...
        verifyNoInteractions(leverageInterestChargeExecutor, leveragePositionRepository);
    }
    ...
    @Test
    @DisplayName("청구에 실패한 포지션은 청구일을 갱신하지 않고, 나머지 포지션은 계속 청구한다")
    void whenOneFails_continuesOthers() {
        LocalDate lastCharged = LocalDate.now().minusDays(1);
        LeveragePositionEntity failing = position(1L, "000660", 365_000L, lastCharged);
        LeveragePositionEntity success = position(2L, "005930", 365_000L, lastCharged);
        willThrow(new IllegalStateException("Account not found"))
                .given(leverageInterestChargeExecutor)
                .chargeInterestForOnePosition(USERNAME, "000660", 1L, 85L);

        int charged = service.chargeInterestForAllPositions(List.of(failing, success));

        assertThat(charged).isEqualTo(1);
        assertThat(failing.getLastInterestChargedDate()).isEqualTo(lastCharged);
        verify(leveragePositionRepository, never()).save(failing);
        verify(leverageInterestChargeExecutor).chargeInterestForOnePosition(USERNAME, "005930", 2L, 85L);
        verify(leveragePositionRepository).save(success);
    }
}
