...
@Slf4j
public class MarketMainSnapshotRepository {

    private static final String SNAPSHOT_KEY = "market:main:snapshot";
    private static final Duration TTL = Duration.ofSeconds(30);

    private final StringRedisTemplate redisTemplate;
    private final ObjectMapper objectMapper;

    public void save(MarketMainResponse response) {
        try {
            String json = objectMapper.writeValueAsString(response);
            redisTemplate.opsForValue().set(SNAPSHOT_KEY, json, TTL);
        } catch (Exception e) {
            log.warn("MarketMainSnapshot 캐시 저장 실패", e);
        }
    }
