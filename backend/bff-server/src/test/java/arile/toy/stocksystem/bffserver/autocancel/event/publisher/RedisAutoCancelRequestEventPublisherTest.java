...
class RedisAutoCancelRequestEventPublisherTest {
    ...
    @Test
    @DisplayName("종목코드로 샤드 스트림을 정하고, 자동 주문 ID·종목코드·요청자 사용자명을 담아 발행한다")
    @SuppressWarnings({"unchecked", "rawtypes"})
    void publishAutoCancel() {
        RedisTemplate<String, Object> redisTemplate = mock(RedisTemplate.class);
        StreamOperations<String, Object, Object> streamOps = mock(StreamOperations.class);
        doReturn(streamOps).when(redisTemplate).opsForStream();
        RedisAutoCancelStreamShardResolver shardResolver = mock(RedisAutoCancelStreamShardResolver.class);
        given(shardResolver.resolveStreamKey("005930")).willReturn("auto-cancel-events-A");
        given(streamOps.add(any(MapRecord.class))).willReturn(RecordId.of("1-0"));

        new RedisAutoCancelRequestEventPublisher(redisTemplate, shardResolver)
                .publishAutoCancel(new AutoCancelRequestEvent(1L, "005930", "user1"));

        ArgumentCaptor<MapRecord> captor = ArgumentCaptor.forClass(MapRecord.class);
        verify(streamOps).add(captor.capture());
        assertThat(captor.getValue().getStream()).isEqualTo("auto-cancel-events-A");
