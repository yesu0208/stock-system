...
class RedisAlertCancelRequestEventPublisherTest {

    @Test
    @DisplayName("종목코드로 샤드 스트림을 정하고, 알림 ID·종목코드·요청자 사용자명을 담아 발행한다")
    @SuppressWarnings({"unchecked", "rawtypes"})
    void publishAlertCancel() {
        RedisTemplate<String, Object> redisTemplate = mock(RedisTemplate.class);
        StreamOperations<String, Object, Object> streamOps = mock(StreamOperations.class);
        doReturn(streamOps).when(redisTemplate).opsForStream();
        RedisAlertCancelStreamShardResolver shardResolver = mock(RedisAlertCancelStreamShardResolver.class);
        given(shardResolver.resolveStreamKey("005930")).willReturn("alert-cancel-events-A");
        given(streamOps.add(any(MapRecord.class))).willReturn(RecordId.of("1-0"));

        new RedisAlertCancelRequestEventPublisher(redisTemplate, shardResolver)
                .publishAlertCancel(new AlertCancelRequestEvent(1L, "005930", "user1"));

        ArgumentCaptor<MapRecord> captor = ArgumentCaptor.forClass(MapRecord.class);
        verify(streamOps).add(captor.capture());
        assertThat(captor.getValue().getStream()).isEqualTo("alert-cancel-events-A");
