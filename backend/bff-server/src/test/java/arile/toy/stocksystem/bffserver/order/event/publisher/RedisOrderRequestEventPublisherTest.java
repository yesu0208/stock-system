...
class RedisOrderRequestEventPublisherTest {
    ...
    @Test
    @DisplayName("종목코드로 샤드 스트림을 정하고, 주문 정보를 문자열 필드로 담아 발행한다")
    @SuppressWarnings({"unchecked", "rawtypes"})
    void publishOrder() {
        RedisTemplate<String, Object> redisTemplate = mock(RedisTemplate.class);
        StreamOperations<String, Object, Object> streamOps = mock(StreamOperations.class);
        doReturn(streamOps).when(redisTemplate).opsForStream();
        RedisOrderStreamShardResolver shardResolver = mock(RedisOrderStreamShardResolver.class);
        given(shardResolver.resolveStreamKey("005930")).willReturn("order-events-A");
        given(streamOps.add(any(MapRecord.class))).willReturn(RecordId.of("1-0"));

        new RedisOrderRequestEventPublisher(redisTemplate, shardResolver).publishOrder(new OrderRequestEvent(
                "user1", "005930", OrderType.BUY, 70_000, 10, LeverageRatio.X2, OrderExecutionType.LIMIT));

        ArgumentCaptor<MapRecord> captor = ArgumentCaptor.forClass(MapRecord.class);
        verify(streamOps).add(captor.capture());
        assertThat(captor.getValue().getStream()).isEqualTo("order-events-A");
